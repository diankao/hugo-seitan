...
标准层面只保证**非递减**的链条：`sizeof(char) <= sizeof(short) <= sizeof(int) <= sizeof(long) <= sizeof(long long)`，且 `sizeof(float) <= sizeof(double)`。`long` 是平台差异最大的一个（4 或 8），凡是把它写进严格不等式的排序都要先确认数据模型（ILP32/LP64/LLP64）。
