...
4. I2C总线被拉低如何恢复？
5. I2C仲裁机制是什么？
6. I2C的地址机制是怎样的？7位和10位地址有什么区别？同地址器件如何共存？

## 考察点

I2C协议底层时序理解、数据读取流程与ACK/NACK规则、总线异常诊断能力、总线恢复手段、多主机仲裁机制、地址机制与保留地址、10位地址兼容性。
...
### 6. I2C地址机制：7位、10位与同地址冲突

#### 7位地址与16个保留区

7位地址空间共128个，其中16个被规范保留，可用地址 **112 个**——不是"只保留0x00和0x7F两个"：

| 保留区 | 用途 |
|--------|------|
| 0x00 | 广播地址（General Call）/ 起始字节 |
| 0x01 | CBUS 兼容（历史遗留） |
| 0x02~0x03 | 保留（其他总线格式/未来用途） |
| 0x04~0x07 | Hs-mode 主器件码 |
| 0x78~0x7B | **10位地址专用** |
| 0x7C~0x7F | 保留给未来 |

器件地址通常由"芯片固定部分 + 引脚可配置位"构成：MPU6050 的 AD0 接地/接高选择 0x68/0x69，24C02 的 A2A1A0 三个引脚凑出 8 个地址。

#### 10位地址：扩展格式与向后兼容的机关

10位地址把空间扩到 1024 个，地址分两个字节发送：

```
第一字节: 1 1 1 1 0 | A9 A8 | R/W     （高5位固定11110）
第二字节: A7 A6 A5 A4 A3 A2 A1 A0     （低8位）
```

**向后兼容的原理**：11110 开头正好落在 7 位地址空间的 0x78~0x7B 保留区——7位老器件从规范设计之初就被要求"见到 11110xx 不应答"。同一条总线上新器件正常握手、老器件装聋，互不干扰。兼容性不是运气，是保留编码预埋的逃生舱（与 flash 行业的 JEDEC ID/SFDP 同一思想）。

**读10位器件的时序坑**：纯读操作也必须走一次重复起始，因为第一字节的 R/W 位必须先为 W：

```
START → 11110+A9A8+W → ACK → 低8位地址 → ACK → RESTART → 11110+A9A8+R → ACK → 读数据 → NACK → STOP
```

很多主控驱动在这里翻车：Linux 需要显式设置 `I2C_M_TEN` 标志，STM32 是 CR2 的 ADD10 位。现实里 10 位器件很少见，绝大多数器件仍是 7 位。

#### 同地址冲突：三板斧

两个同地址器件挂一条总线是真实工程问题（对比 SPI：片选就是地址，带外解决，无此问题）：

| 方案 | 做法 | 典型例子 |
|------|------|---------|
| 地址引脚 | 拉高/拉低配置位改变地址 | MPU6050 的 AD0、EEPROM 的 A0~A2 |
| I2C多路复用器 | 一分八，每路独立总线分时选通 | TCA9548A |
| 轮询隔离 | 同型器件逐个上电/复位，逐个操作 | 无地址引脚、地址固定的模块 |

#### 规范演化视角：官方修订，不是厂家魔改

I2C 的扩展全部由规范持有者（NXP，UM10204）以修订版形式集中发布：Standard 100kHz（1982）→ Fast 400kHz（1992）→ Hs-mode 3.4MHz（1998）→ Fast-mode Plus 1MHz（2007）→ Ultra Fast-mode 5MHz 单向推挽（2012），以及10位地址——每次扩展靠保留编码向后兼容。对比 SPI 没有标准组织、各家手册自由发挥（QSPI/命令集碎片化）：**I2C 的"魔改"都在规范里，SPI 的魔改都在各家手册里**。
